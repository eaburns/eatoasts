...
;; Created: 8 Sep 1993
;; Modified Dec 5 2008 -- eaburns: fixed syntax highlighting, bumped
;;                                 version to 0.5.
;; Version: 0.5
...
;;
;; To install, put this file in ~/.elisp.d
;;
;; and add the following to you ~/.emacs file
;;
;; (setq load-path
;;      (append load-path
;;              (list (format "%s/.elisp.d" (getenv "HOME")))))
;; (require 'tla-mode)
...
args, if that value is non-nil."
  ...
  (if tla-use-font-lock
       (tla-fontify))
  (if tla-startup-message
      (message "Emacs TLA+ mode %s."
	       tla-mode-version))
...
(defconst tla-font-lock-keywords
  (list
   ...
   "SF"
   "EXCEPT"
   (list "\\\\notin\\|\\\\intersect\\|\\\\subset\\|\\\\subseteq\\|\\\\union\\|\\\\in" 0 'font-lock-keyword-face)
   (list "\\\\geq\\|\\\\leq" 0 'font-lock-keyword-face)
   (list "\\\\A" 0 'font-lock-keyword-face)
   (list "\\\\E" 0 'font-lock-keyword-face)
   ...
   ;; there is a bug in the following line... it highlights the commas too...
   (list "[a-zA-Z0-9_]+[ \t\n]*(\\(.*\\))[ \t]*=="
	 1 'font-lock-variable-name-face)
   ...
   ;; constants -- this must come last so that [0-9] doesn't mess with
   ;; identifiers
   (list "TRUE\\|FALSE\\|BOOLEAN" 0 'font-lock-constant-face)
   (list "[0-9]" 0 'font-lock-constant-face)
   ))
